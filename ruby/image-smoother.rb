# @leetup=custom
# @leetup=info id=661 lang=ruby slug=image-smoother
...
def image_smoother(img)
  adjacent_cells = [-1, 0, 1].then { _1.product(_1) }
  Array.new(img.size) do |i|
    Array.new(img[i].size) do |j|
      adjacent_cells
        .filter_map { [i + _1, j + _2] if i + _1 >= 0 && j + _2 >= 0 }
        .filter_map { img.dig(_1, _2) }
        .then { _1.sum / _1.size }
    end
  end
end
