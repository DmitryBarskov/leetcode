...
def min_difficulty(job_difficulty, total_days)
  recur = memoize ->(previous_day_difficulty, jobs_to_schedule, d) {
    ...
    return recur.call(current, other_jobs, d - 1) if previous_day_difficulty.nil?
    ...
    [
      recur.call([previous_day_difficulty, current].max, other_jobs, d),
      recur.call(current, other_jobs, d - 1)&.+(previous_day_difficulty)
    ].compact.min
  }

  recur.call(nil, job_difficulty, total_days) || -1
end
# @leetup=code
