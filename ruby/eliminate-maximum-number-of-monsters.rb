...
# Output: 3
# Explanation:
# In the beginning, the distances of the monsters are [1,3,4]. You eliminate the f
# irst monster.
# After a minute, the distances of the monsters are [X,2,3]. You eliminate the sec
# ond monster.
# After a minute, the distances of the monsters are [X,X,2]. You eliminate the thr
# id monster.
...
# Output: 1
# Explanation:
# In the beginning, the distances of the monsters are [3,2,4]. You eliminate the f
# irst monster.
...
# @param {Integer[]} speeds
# @return {Integer}
def eliminate_maximum(distances, speeds)
  time_to_city = distances.zip(speeds).map! { |distance, speed| distance.to_f / speed }.sort!
  time_to_city.each_with_index do |time, index|
    return index if time <= index
  end
  time_to_city.size
end
# @leetup=code
