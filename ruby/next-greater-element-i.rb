# @leetup=custom
# @leetup=info id=496 lang=ruby slug=next-greater-element-i
...
def next_greater_elements(nums)
  next_peak = -1
  Array.new(nums.size).tap do |arr|
    prev = nil
    (0...nums.size).reverse_each do |i|
      ...
      prev = [nums[i], prev].compact.min
    end
  end
end

next_greater_elements([1, 2, 5, 4, 3, 6]) => [2, 5, 6, 6, 6, -1]
next_greater_elements([3, 8, 4, 5, 2, 1]) => [8, -1, 5, -1, -1, -1]
next_greater_elements([1, 3, 2, 5, 4, 8, 7, 6]) => [3, 5, 5, 8, 8, -1, -1]
next_greater_elements([1, 3, 4, 2]) => [3, 4, -1, -1]
next_greater_elements([6, 5, 4, 3, 2, 1, 7]) => [7, 7, 7, 7, 7, 7, 7, -1]
