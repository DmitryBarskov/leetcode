...
def arithmetic_triplets(nums, diff)
  ...
  while i < nums.length && j < nums.length && k < nums.length
    j = find_next_with_diff(nums, from: i, diff:, starting_position: j)
    break if nums.length <= j
    next i += 1 if nums[i] + diff < nums[j]

    k = find_next_with_diff(nums, from: j, diff:, starting_position: k)
...
def find_next_with_diff(array, from:, diff:, starting_position:)
  j = starting_position
