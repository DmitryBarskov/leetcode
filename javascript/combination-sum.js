// @leetup=custom
// @leetup=info id=39 lang=javascript slug=combination-sum
 ...
 * @return {number[][]}
 */
function combinationSum(candidates, target, start = 0) {
  if (target < 0) return [];
  if (target === 0) return [[]];
  if (start >= candidates.length) return [];

  return candidates.slice(start).flatMap((candidate, index) =>
    combinationSum(candidates, target - candidate, start + index).map(
      (prev) => [candidate, ...prev]
    )
  );
}
