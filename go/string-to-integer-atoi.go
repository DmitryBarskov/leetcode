// @leetup=custom
// @leetup=info id=8 lang=golang slug=string-to-integer-atoi
...
* Output: 4193
* Explanation:
* Step 1: "4193 with words" (no characters read because there is no leading whites
* pace)
*          ^
* Step 2: "4193 with words" (no characters read because there is neither a '-' nor
*  '+')
*          ^
* Step 3: "4193 with words" ("4193" is read in; reading stops because the next cha
* racter is a non-digit)
*              ^
...
func myAtoi(s string) int {
	begin := indexOfFirst(s, notWhitespace, 0)
	end := indexOfFirst(s, notIntRune, begin)
	intAsString := s[begin:end]
	if validInt(intAsString) {
		return 0
	}
	var number int = intFromString(intAsString)
	return number
}
...
func intFromString(s string) int {
	...
	unsignedValue, overflow := positiveIntFromString(unsigned)
	if overflow && negative {
		return MIN_INT_VALUE
	} else if overflow {
		return MAX_INT_VALUE
	} else {
		return sign * int(unsignedValue)
	}
}

func positiveIntFromString(unsigned string) (value int32, overflow bool) {
	var intValue int32 = 0
	for _, digit := range unsigned {
		nextIntValue := intValue*10 + digitFromRune(digit)
...
func digitFromRune(r rune) int32 {
	return int32(r) - '0'
}

func indexOfFirst(s string, predicate func(r rune) bool, startFrom int) int {
