// @leetup=custom
// @leetup=info id=1846 lang=typescript slug=maximum-element-after-decreasing-and-rearranging
...
*   equal to `1`. In other words, `abs(arr[i] - arr[i - 1]) <= 1` for each `i`
...
* Input: arr = [1,2,3,4,5]
* Output: 5
* Explanation: The array already satisfies the conditions, and the largest ele
* ment is 5.
* 
* 
* Constraints:
...
function maximumElementAfterDecrementingAndRearranging(arr: number[]): number {
  arr.sort((a, b) => a - b);
  return arr.reduce((prev, curr) => Math.min(prev + 1, curr), 0);
}
// @leetup=code
