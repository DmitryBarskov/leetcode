// @leetup=custom
// @leetup=info id=606 lang=java slug=construct-string-from-binary-tree
    ...
    public static void main(String[] args) {
        System.out.println(
            new Solution().tree2str(
                TreeNode.fromArray(new Integer[] {1, 2, 3, 4})
            )
        );
        System.out.println(
            new Solution().tree2str(
                TreeNode.fromArray(new Integer[] {1, 2, 3, null, 4})
            )
        );
    }
}
...
class Solution {
    public static String tree2str(TreeNode root) {
        ...
        if (root.left == null && root.right == null) {
            return String.format("%d", root.val);
        }
        if (root.right == null) {
