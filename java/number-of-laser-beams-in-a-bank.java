// @leetup=custom
// @leetup=info id=2125 lang=java slug=number-of-laser-beams-in-a-bank
...
*  row.
* This is because the 2nd row contains security devices, which breaks the second c
* ondition.
* 
* Example 2:
* 
...
class Solution {
    public int numberOfBeams(String[] bank) {
        ...
        int totalBeams = 0;
        for (String row : bank) {
            int devices = countOnes(row);
            if (devices == 0) {
                continue;
            }

            totalBeams += previousRowDevices * devices;
            previousRowDevices = devices;
        }
        return totalBeams;
    }

    static int countOnes(String string) {
        int count = 0;
