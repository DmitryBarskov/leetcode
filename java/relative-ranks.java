// @leetup=custom
// @leetup=info id=506 lang=java slug=relative-ranks
...
class Solution {
    public String[] findRelativeRanks(int[] score) {
        ...
        Arrays.sort(sorted);
        var ranks = new HashMap<Integer, Integer>(sorted.length);
        for (int i = 0; i < sorted.length; i++) {
            ranks.put(sorted[i], sorted.length - i);
        }
        String[] ranked = new String[score.length];
        for (int i = 0; i < score.length; i++) {
            ...
            if (rank == 1) {
                ...
                ranked[i] = Integer.toString(rank);
            }
        }
        return ranked;
    }
